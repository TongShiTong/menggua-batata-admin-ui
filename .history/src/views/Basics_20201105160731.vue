                        ...
                        <div v-if="provIndex===index&&cityIndex1===index1">
                          <div v-for="(item2, index2) in areaList" :key="index2" class="area-box"  @click="bindArea(index2)">
                            ...
                            <div class="gs-times" style="width:200px">
                              ...
                              <input type="text" placeholder="编号" onkeypress="if(window.event.keyCode==13) this.blur()" @blur="regionNum" v-model="item2.regionNum" style="width:40%;text-align:left;background:none;">
                            </div>
        ...
        if(res.code=="801") {
          ...
          for(let i=0; i<res.result.length; i++) {
            if(res.result[i].startTime && res.result[i].endTime) {
              let allTime = []
              let getScale = Number(this.input2 * res.result[i].actualScale/100)
              let supplementScale = Number( Math.floor(this.input2 * res.result[i].actualScale * res.result[i].regionScale) / 100)
              allTime.push(res.result[i].startTime)
              allTime.push(res.result[i].endTime)
              res.result[i].getScale = getScale
              res.result[i].supplementScale = supplementScale
