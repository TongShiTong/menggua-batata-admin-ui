                      ...
                      <div v-for="(item1, index1) in item.city" :key="index1">
                        ...
                        <div v-if="provIndex===index&&cityIndex1===index1">
                          <div v-for="(item2, index2) in areaList" :key="index2" class="area-box"  @click="bindArea(index2)">
                            ...
                            <div class="input-box">
                              <input style="background:none;" type="text" placeholder="0" v-model="item2.getScale" disabled/>%
                            </div>
                            <div class="input-box">
                              <input type="text" placeholder="0" onkeypress="if(window.event.keyCode==13) this.blur()" @blur="regionScale" v-model="item2.regionScale" />%
                            </div>
                            <div class="input-box">
                              <input style="background:none;" type="text" placeholder="0" v-model="item2.supplementScale" disabled />%
                            </div>
  ...
  methods: {
      changeTime(e) {
        ...
        if(e) {
          ...
          this.$message.warning('请选择该区域归属时间！')
        }
      ...
      actualScale() {
       let data = this.areaList[this.areaIndex]
       let data1 = this.originArea.result[this.areaIndex]
        if(data1.actualScale === data.actualScale) {
          return
        }else {
          data1.actualScale = data.actualScale
        }
      ...
      regionScale() {
       let data = this.areaList[this.areaIndex]
       let data1 = this.originArea.result[this.areaIndex]
        if(data1.regionScale === data.regionScale) {
          return
        }else {
          data1.regionScale = data.regionScale
        }
      ...
      regionName() {
       let data = this.areaList[this.areaIndex]
       let data1 = this.originArea.result[this.areaIndex]
        if(data1.regionName === data.regionName) {
          return
        }else {
          data1.regionName = data.regionName
        }
        if(data.regionName==='') {
      ...
      regionNum() {
       let data = this.areaList[this.areaIndex]
       let data1 = this.originArea.result[this.areaIndex]
        if(data1.regionNum === data.regionNum) {
          return
        }else {
          data1.regionNum = data.regionNum
        }
        if(data.regionNum==='') {
         data.regionNum = null
       }
      ...
      async getArea(id) {
        ...
        if(res.code=="801") {
          ...
          this.originArea = JSON.parse(JSON.stringify(res))
          this.cityIndex1 = this.cityIndex
          console.log(111,this.areaList)
          console.log(222,this.originArea)
        }else if(res.code=="802") {
