                        ...
                        <div v-if="provIndex===index&&cityIndex1===index1">
                          <div v-for="(item2, index2) in areaList" :key="index2" class="area-box"  @click="bindArea(index2)">
                            ...
                            <div class="gs-times" style="width:200px">
                              <input type="text" placeholder="姓名" onkeypress="if(window.event.keyCode==13) this.blur()" @blur="regionName" v-model="item2.regionName" style="width:35%;text-align:right;background:none;">
                              <input type="text" placeholder="帐号" onkeypress="if(window.event.keyCode==13) this.blur()" @blur="regionNum" v-model="item2.regionNum" style="width:54%;text-align:left;background:none;">
                            </div>
...
export default {
  ...
  // watch: {
  //   input1: function (newValue, oldval) {
  //     console.log(newValue, oldval)
  //   },
  //   input2: function (newValue, oldval) {
  //     console.log(newValue, oldval)
  //   }
  // },
  //事件处理函数
  methods: {
      ...
      upInput1() {
        if(this.orginValue[0].value === this.input1) {
          return
        }else {
          this.orginValue[0].value = this.input1
        }
        if(this.input1==='') {
          this.input1 = 0
        }
        ...
        if(this.input3<0) {
          this.$message.error('修改失败,分配比例和为100%')
          return
        }
        let data = {
          itemCommission:this.input1,
          regionalDividend:this.input2,
          platformSurplus: this.input3
        }

        this.modifyInfo1(data)
      },
      upInput2() {
        if(this.orginValue[1].value === this.input2) {
          return
        }else {
          this.orginValue[1].value = this.input2
        }
        if(this.input2==='') {
          this.input2 = 0
        }
        ...
        this.getArea1( this.dataList[this.provIndex].city[this.cityIndex].id)
      },
      // upInput3() {
      //   if(this.input3==='') {
      //     this.input3 = 0
      ...
      // },
      upInput4() {
        if(this.orginValue[3].value === this.input4) {
          return
        }else {
          this.orginValue[3].value = this.input4
        }
        if(this.input4==='') {
          this.input4 = 0
        }
      ...
      actualScale() {
        ...
        if(this.areaList1[this.areaIndex].actualScale === this.areaList[this.areaIndex].actualScale) {
          return
        }else {
          this.areaList1[this.areaIndex].actualScale = this.areaList[this.areaIndex].actualScale
        }
       let data = this.areaList[this.areaIndex]
      ...
      regionScale() {
        if(this.areaList1[this.areaIndex].regionScale === this.areaList[this.areaIndex].regionScale) {
          return
        }else {
          this.areaList1[this.areaIndex].regionScale = this.areaList[this.areaIndex].regionScale
        }
        let data = this.areaList[this.areaIndex]
        if(data.regionScale==='') {
         data.regionScale = 0
      ...
      regionName() {
        if(this.areaList1[this.areaIndex].regionName === this.areaList[this.areaIndex].regionName) {
          return
        }else {
          this.areaList1[this.areaIndex].regionName = this.areaList[this.areaIndex].regionName
        }
        let data = this.areaList[this.areaIndex]
        if(data.regionName==='') {
      ...
      regionNum() {
        if(this.areaList1[this.areaIndex].regionNum === this.areaList[this.areaIndex].regionNum) {
          return
        }else {
          this.areaList1[this.areaIndex].regionNum = this.areaList[this.areaIndex].regionNum
        }
        let data = this.areaList[this.areaIndex]
        if(data.regionNum==='') {
         data.regionNum = null
      ...
      async getArea1(id) {
        let self = this
        const { data: res } = await this.$http.get("/district/comper", { params: {id} });
        if(res.code=="801") {
          this.getArea(id)
        }else if(res.code=="802") {
      ...
      // 批量更新商品默认利润字段的值接口
      async modifyInfo1(data) {
        const { data: res } = await this.$http.get("/itemSubsidy/updateSpecialFieldsValueBatch", { params: data});
        if(res.code=="801") {
          this.$message.success('修改信息成功')
