...
<!-- v-loading="listLoading" el-table加载属性-->
    ...
    >
      ...
      <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
					<el-button type="primary" size="small" :disabled="row.patternStatus==1?false:true" @click="handleUpdate(row)">编辑</el-button>
					<el-button type="danger" size="small" :disabled="row.patternStatus==1?false:true" @click="handleDelete(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="'新增任务'" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm1" :rules="rules" :model="this.tasks" label-position="left" label-width="100px" style="width: 500px; margin-left:50px;">
		...
		<el-form-item label="派发方式" prop="distributeType">
          <el-select v-model="tasks.distributeType" placeholder="请选择派发方式">
						<el-option
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
		...
		distributeType: [{ required: true, message: '请选择派发方式', trigger: 'blur' }],
  ...
  methods: {
		...
		async getPattern(){
			const res = await this.$http.get('/pattern/getPatternByState')
			// console.log(res)
			if(res.data.code==801){
			...
			this.$refs['dataForm1'].validate((valid) => {
				if(valid){
					...
					const tasksData = {
						...
						distributeType: this.tasks.distributeType,
...
//             },
    ...
    handleUpdate(row) {
			...
			this.tasks.distributeType = row.distributeType,
