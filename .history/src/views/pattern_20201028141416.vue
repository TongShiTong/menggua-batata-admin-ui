<template>
  <div class="app-container">
    ...
    <el-table :data="patternList" fit highlight-current-row style="width: 200%;">
      ...
      <el-table-column label="操作" align="center" min-width="300px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          ...
          <el-button type="primary" size="small" icon="el-icon-d-arrow-right" @click="getLevelList(row.pattType)">
            跳转等级详情
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-drawer
      ...
      size="60%">
       ...
       <el-table :data="levelList" style="width:100%">
         ...
         <el-table-column label="等级权益" min-width="80px" align="center">
           <template slot-scope="{row}">
         ...
         <el-table-column label="升级条件" min-width="80px" align="center">
           <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="addPatternlogVisible" width="50%">
      <el-form ref="addPatternForm" :model="Pattern" :rules="addPatterRules" status-icon label-position="left" label-width="80px" style="width: 300px; margin-left:30px;">
        <el-form-item label="模式名称" prop="designation">
          <el-input v-model="Pattern.designation"/>
        </el-form-item>
        ...
        <el-form-item label="模式周期" prop="timeRange">
          <el-date-picker
                @change="findPatternCycle(Pattern.pattType)"
                v-model="timeRange"
                type="datetimerange"
                range-separator="至"
    ...
    <!-- 添加等级对话框-->
    <el-dialog :title="textMap1[dialogStatus1]" :visible.sync="addLevellogVisible" width="30%">
    ...
    <el-dialog title="权益设置" :visible.sync="dialogEquityVisible">
      ...
      <el-table :data="equtiyList">
        <el-table-column property="date" label="权益名称"ninwidth="150">
          <template slot-scope="{row}">
      ...
      <el-form ref="levelEquitylRef" :model="levelEquity" :rules="addEquityRules" status-icon label-position="left">
        ...
        <el-form-item label="权益数值" prop="amount" style="margin-left:15px ;">
            <el-input  size="mini" style="width:130px;" v-model="levelEquity.amount"  placeholder="请输入权益匹配值"/>
        </el-form-item>
    ...
    <el-dialog title="升级条件设置" :visible.sync="dialogUpConstionVisible">
      ...
      <el-table :data="LevelSepList">
        <el-table-column property="date" label="等级名称"ninwidth="160">
          <template slot-scope="{row}">
    ...
    methods: {
      ...
      async getPatternList() {
        ...
        if (res.code == "801") {
          ...
          console.log(res.result.patternANDlevel)
      ...
      async getLevelSep() {
        const {
          data: res
        } = await this.$http.get('/upgrade/getUpgrade',  {params: {pattType:this.patte}})
        console.log(res)
        if (res.code == "801") {
      ...
      addPattern(){
          ...
          this.$refs.addPatternForm.validate(async valid => {
            ...
            if (res.code !== "802") {
              return this.$message.error('模式创建失败！')
            }
          })
      },
      ...
      updateLevel(leveld){
        ...
        this.$refs.addLevelRef.validate(async valid => {
          ...
          console.log(  this.Level.levelId )
          const {
            data: res
      ...
      updatePattern(pattid){
        ...
        this.$refs.addPatternForm.validate(async valid => {
          ...
          console.log( this.Pattern.pattId )
      ...
      async getWhereBylevelId(levelId){
        ...
        console.log(this.LevelSep.levelId)
        const {
          data: res
      ...
      async changeState(pattId){
           ...
           const {
            ...
            console.log(this.patternTypeDate)
           console.log(res)
           if (res.code == "801") {
             this.getPatternList()
             return
      ...
      /* 自动升级*/
       ...
       if (res.code == "8001") {
         return this.$message.success('自动升级成功,请到业绩模块查看！')
       }
       if (res.code !== "8001") {
         return this.$message.error('自动升级成功请重试！')
       }
      },
      ...
      /*添加模式时校验周期*/
       async findPatternCycle(pattType){
        console.log(pattType)
        for (let i = 0; i < this.timeRange.length; i++) {
          if(i == 0){
            this.Pattern.startTime=parseTime( this.timeRange[i])
            console.log('开始'+this.Pattern.startTime)
        ...
        const {
          data: res
        } = await this.$http.get('/pattern/getPatternCycle', {params: {userType:pattType,startTime:this.Pattern.startTime,endTime:this.Pattern.endTime}})
        if (res.code !== "801") {
