<template>
  <div class="app-containers-box">
    <div class="home-box">
      ...
      <div class="save-btn">
        <el-button type="primary" size="small" round @click="saveItem(1)">发布</el-button>
      </div>
    </div>
    <div class="home-box">
...
export default {
  ...
  methods: {
    ...
    regionAvatarSuccessBg(res, file) {
      this.region.title_img = res.result
    },
    cloudAvatarSuccessBg(res, file) {
      this.cloud.title_img = res.result
    },
    // 横幅上传
    weeklyHengfSuccessBg(res, file) {
      this.weekly.banner_info.img_url = res.result
    },
    regionHengfSuccessBg(res, file) {
      this.region.banner_info.img_url = res.result
    },
    cloudHengfSuccessBg(res, file) {
      this.cloud.banner_info.img_url = res.result
    },
    ...
    deleteTitle(type) {
      if(type===1) {
        ...
        this.region.title_img = "";
      }else if(type===3) {
        this.cloud.title_img = "";
      }
    },
    ...
    deleteHengfu(type) {
      if(type===1) {
        ...
        this.region.banner_info.img_url = "";
      }else if(type===3) {
        this.cloud.banner_info.img_url = "";
      }
    },

    async getHomeData() {
      const { data: res } = await this.$http.post("/firstPage/getfirstPageConfiguration",data);
      if(res.code=="801") {
    ...
    saveItem(type) {
      ...
      if(type===1) {
        for(let i=0; i<this.bannerList.length; i++) {
          data.str[i].banner =  this.bannerList[i].img_url
          data.str[i].bannerUrl = this.bannerList[i].link
        }
